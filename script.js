...
function nextRound(card) {
  ...
  while (
    mainCard.textContent != mainCard2.textContent &&
    mainCard.style.background != mainCard2.style.background
  ) {
    mainCard.innerText = Math.floor(Math.random() * 6) + 1;
    mainCard.style.background = cardColors[Math.floor(Math.random() * 4)];
    actualRound = false;
...
function checkCard(card) {
  if (
    card.textContent === mainCard.textContent ||
    card.style.background === mainCard.style.background
  ) {
